...
func PublicSettings() (model.PublicSetting, error) {
	...
	pub := normalizePublicSetting(normalized.Public)
	// 从私有配置派生注册状态到公开配置
	pub.Auth.AllowRegister = normalized.Private.Auth.AllowRegister == nil || *normalized.Private.Auth.AllowRegister
	pub.Auth.AllowPasswordRegister = normalized.Private.Auth.AllowPasswordRegister == nil || *normalized.Private.Auth.AllowPasswordRegister
	return pub, nil
}
...
func normalizePrivateSetting(setting model.PrivateSetting) model.PrivateSetting {
	...
	if setting.Auth.AllowPasswordRegister == nil {
		enabled := true
		setting.Auth.AllowPasswordRegister = &enabled
	}
	return setting
}
