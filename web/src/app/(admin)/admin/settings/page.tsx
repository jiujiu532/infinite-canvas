                        ...
                        <Form form={form} layout="vertical" initialValues={emptySettings} requiredMark={false}>
                            <Flex vertical gap={12}>
                                ...
                                <Card size="small" title="注册控制">
                                    <Row gutter={16}>
                                        <Col xs={24} md={12}>
                                            <Form.Item name={["private", "auth", "allowRegister"]} label="允许注册" extra="关闭后所有注册方式均不可用，包括 LinuxDo 新用户" valuePropName="checked">
                                                <Switch />
                                            </Form.Item>
                                        </Col>
                                        <Col xs={24} md={12}>
                                            <Form.Item name={["private", "auth", "allowPasswordRegister"]} label="允许密码注册" extra="关闭后仅禁止账号密码注册，LinuxDo 等第三方登录仍可注册新用户" valuePropName="checked">
                                                <Switch />
                                            </Form.Item>
                                        </Col>
                                    </Row>
                                </Card>
                                <Card size="small" title="签到与注册算力点">
                                    <Row gutter={16}>
                                        <Col xs={24} md={8}>
                                            <Form.Item name={["private", "checkin", "minCredits"]} label="签到最小算力点" extra="每日签到随机获得的最小值">
                                                <InputNumber min={0} step={1} precision={0} className="!w-full" />
                                            </Form.Item>
                                        </Col>
                                        <Col xs={24} md={8}>
                                            <Form.Item name={["private", "checkin", "maxCredits"]} label="签到最大算力点" extra="每日签到随机获得的最大值">
                                                <InputNumber min={0} step={1} precision={0} className="!w-full" />
                                            </Form.Item>
                                        </Col>
                                        <Col xs={24} md={8}>
                                            <Form.Item name={["private", "registerCredits", "initialCredits"]} label="注册初始算力点" extra="新用户注册时赠送的算力点">
                                                <InputNumber min={0} step={1} precision={0} className="!w-full" />
                                            </Form.Item>
                                        </Col>
                                    </Row>
...
function normalizePublicSetting(setting: Partial<AdminSettings["public"]> = {}): AdminSettings["public"] {
    return {
        ...
        auth: {
            ...
            allowPasswordRegister: setting.auth?.allowPasswordRegister !== false,
            linuxDo: {
...
function normalizePrivateSetting(setting: Partial<AdminSettings["private"]> = {}): AdminSettings["private"] {
    return {
        ...
        checkin: {
            minCredits: Math.max(0, Number(setting.checkin?.minCredits) || 1),
            maxCredits: Math.max(0, Number(setting.checkin?.maxCredits) || 10),
        },
        registerCredits: {
            initialCredits: Math.max(0, Number(setting.registerCredits?.initialCredits) || 0),
        },
    };
}
